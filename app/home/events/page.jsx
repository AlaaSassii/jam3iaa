...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const filtered = events.filter((event) => {
        ...
        const eventDescription = event.description ? event.description.toLowerCase() : ""; // Check if event description exists
      ...
      <div className="flex flex-col justify-center items-center gap-5 mt-20">
        <div>
          <label className="input input-bordered flex items-center gap-2">
            <input type="text" className="grow" placeholder="Search" onChange={(e) => setSearchQuery(e.target.value)} value={searchQuery} />
